...
write_boot() {
  ...
  if [ "$(getprop ro.product.device)" == gemini ]; then
    for i in zImage; do
      if [ -f /tmp/anykernel/gemini/$i ]; then
        kernel=/tmp/anykernel/gemini/$i;
        break;
      fi;
    done;
  fi;
  if [ "$(getprop ro.product.device)" == capricorn ]; then
    for i in zImage; do
      if [ -f /tmp/anykernel/capricorn/$i ]; then
        kernel=/tmp/anykernel/capricorn/$i;
        break;
      fi;
    done;
  fi;
  if [ "$(getprop ro.product.device)" == natrium ]; then
    for i in zImage; do
      if [ -f /tmp/anykernel/natrium/$i ]; then
        kernel=/tmp/anykernel/natrium/$i;
        break;
      fi;
    done;
  fi;
  if [ "$(getprop ro.product.device)" == lithium ]; then
    for i in zImage; do
      if [ -f /tmp/anykernel/lithium/$i ]; then
        kernel=/tmp/anykernel/lithium/$i;
        break;
      fi;
    done;
  fi;
  if [ "$(getprop ro.product.device)" == scorpio ]; then
    for i in zImage; do
      if [ -f /tmp/anykernel/scorpio/$i ]; then
        kernel=/tmp/anykernel/scorpio/$i;
        break;
      fi;
    done;
  fi;
  ...
  if [ "$(getprop ro.product.device)" == capricorn ]; then
    for i in dtb; do
      if [ -f /tmp/anykernel/capricorn/$i ]; then
        dtb="--dt /tmp/anykernel/capricorn/$i";
        break;
      fi;
    done;
  fi;
  if [ "$(getprop ro.product.device)" == natrium ]; then
    for i in dtb; do
      if [ -f /tmp/anykernel/natrium/$i ]; then
        dtb="--dt /tmp/anykernel/natrium/$i";
        break;
      fi;
    done;
  fi;
  if [ "$(getprop ro.product.device)" == lithium ]; then
    for i in dtb; do
      if [ -f /tmp/anykernel/lithium/$i ]; then
        dtb="--dt /tmp/anykernel/lithium/$i";
        break;
      fi;
    done;
  fi;
  if [ "$(getprop ro.product.device)" == scorpio ]; then
    for i in dtb; do
      if [ -f /tmp/anykernel/scorpio/$i ]; then
        dtb="--dt /tmp/anykernel/scorpio/$i";
        break;
      fi;
    done;
  fi;
